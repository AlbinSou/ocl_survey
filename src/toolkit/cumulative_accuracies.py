...
class CumulativeAccuracyPluginMetric(
    ...
    def before_training_exp(self, strategy, **kwargs):
        ...
        # from parallel eval plugins (for which training callbacks are not called)
        
        strategy.clock._classes_splits = self.classes_splits
    ...
    def before_eval(self, strategy, **kwargs):
        # Update classes splits by chosing 
        # the one with more entries
        super().before_eval(strategy, **kwargs)
        if hasattr(strategy.clock, "_classes_splits"):
            self.classes_splits = strategy.clock._classes_splits
